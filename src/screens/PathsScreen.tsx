...
// Tela 6 - Diagrama de caminhos 3x2
export default function PathsScreen({ onNext }: { onNext: () => void }) {
  ...
  const startDrag = (id: string) => (e: React.PointerEvent) => {
    ...
    // Track move via window listener
    ...
    const up = (ev: PointerEvent) => {
      ...
      if (!bodyId) {
        setFeedback({ msg: "Ligue uma cabeça a um corpo. Assim formamos um caminho de combinação.", tone: "warn" });
      } else {
        addPath(id, bodyId);
      }
      setDragging(null);
  ...
  const addPath = (headId: string, bodyId: string) => {
    ...
    if (paths.has(key)) {
      setFeedback({ msg: "Esse caminho já foi feito. Procure uma ligação que ainda falta.", tone: "warn" });
      return;
    }
    const next = new Set(paths); next.add(key);
    setPaths(next);
    // Verifica se essa cabeça já ligou aos 2 corpos
    const countForHead = [...next].filter(p => p.startsWith(headId + "__")).length;
    if (next.size === total) {
      setFeedback({
        msg: "Você encontrou os 6 caminhos. Cada uma das 3 cabeças se ligou aos 2 corpos: 3 × 2 = 6.",
        tone: "success",
      });
    } else if (countForHead === 2) {
      setFeedback({
        msg: "Boa! Essa cabeça já combinou com os 2 corpos. Agora escolha outra cabeça.",
        tone: "success",
      });
    }
  };
  ...
  return (
    <ScreenShell
      title="Diagrama de caminhos (3 × 2)"
      subtitle="Ligue cada cabeça a cada corpo arrastando."
    >
      <div style={{
        position: "absolute", top: 110, left: 200, right: 30, bottom: 100,
        background: "rgba(255,255,255,0.9)", borderRadius: 18,
        border: "3px solid #1e293b", padding: 16,
          ...
          <div ref={areaRef}
            ...
            <svg width={W} height={H} style={{
              ...
              {[...paths].map((key, i) => {
                ...
                const a = headPositions[h], c = bodyPositions[b];
                return <line key={key} x1={a.x} y1={a.y} x2={c.x} y2={c.y}
                  stroke={colors[i % colors.length]} strokeWidth={4} />;
              })}
              {dragging && (
                <line x1={headPositions[dragging.from].x} y1={headPositions[dragging.from].y}
                  x2={dragging.x} y2={dragging.y} stroke="#64748b" strokeDasharray="6 6" strokeWidth={3} />
              )}
            </svg>
            ...
            {heads.map(h => {
              ...
              return (
                <div key={h.id}
                  ...
                  style={{
                    position: "absolute", left: p.x - 35, top: p.y - 35,
                    width: 70, height: 70, borderRadius: 16,
                    background: "white", border: "3px solid #f59e0b",
                    display: "flex", alignItems: "center", justifyContent: "center",
                    cursor: "grab", touchAction: "none", userSelect: "none",
                  }}>
                  <img src={h.imagem} alt={h.nome} draggable={false}
            ...
            {bodies.map(b => {
              ...
              return (
                <div key={b.id} data-bodyid={b.id}
                  style={{
                    position: "absolute", left: p.x - 40, top: p.y - 45,
                    width: 80, height: 90, borderRadius: 16,
                    background: "white", border: "3px solid #3b82f6",
                    display: "flex", alignItems: "center", justifyContent: "center",
                  }}>
                  <img src={b.imagem} alt={b.nome} draggable={false}
                    data-bodyid={b.id}
